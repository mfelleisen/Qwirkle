...
;; ---------------------------------------------------------------------------------------------------
(provide red link-color blue green yellow black orange pink purple background-white)
...
(define-syntax (define-n stx)
  (syntax-parse stx
    [(_)
     #:do [(define here stx)
           (define loc  (syntax-source here))
           (define-values [base name is-dir?] (split-path loc))]
     #:with n (datum->syntax stx 'n)
     #:with o (datum->syntax stx 'other-files)
     #:with x (datum->syntax stx (cadr (regexp-match #px"(.*)\\.scrbl" name)))
     (syntax/loc stx
       (begin
	 (define (o)
	   @t{All auxiliary files must be put into a sub-directory called @(tt n "/Other/").})
	 (define n x)))]))

;; -----------------------------------------------------------------------------

(provide require-constants)

(require (for-syntax (only-in racket ~a)))

(define-syntax (require-constants stx)
  (syntax-parse stx
    [(_ [path constant:id ...] ...)
     #:with ((constant-uc ...) ...)
     (for/list ([row (syntax->list #'((constant ...) ...))])
       (for/list ([x (syntax->list row)])
         (format-id stx "~a" (string-upcase (~a (syntax-e x))))))

 
     #'(begin
	 (require (only-in path constant-uc ...) ...)
	 (define constant
...
(provide keep-in-mind delivery size-constraint test-is valid)

(define [keep-in-mind]
  @t{Keep in mind our @secref{plan} while you work on a design task.})

(define [delivery]
  @t{@blue{Delivery} Place the product of this week's milestone into your git repo as follows:})

(define [size-constraint]
  @t{@bold{Constraint} No test file may exceed the size limit of 20Kb.})

(define [test-is]
  @t{A test case always consists of given inputs and expected outputs. For this
course, a test consists of a pair of files: @tt{n-in.json}, the input file, and
@tt{n-out.json}, the expected output file, where @tt{n} is an integer between 0
and the requested number of tests (exclusive).---@bold{Constraint} No test file 
may exceed the size limit of 20Kb.})

(define [valid #:special (special "")]
  @t{@bold{Well-formed and Valid} You may assume that all @special inputs to your
test harnesses from @stdin[] are well-formed and valid.  A @tech{well-formed} piece of JSON
satisfies the grammar; such a piece is @deftech{valid} if it also satisfies all
the side constraints of a schema specification. })

@; -----------------------------------------------------------------------------

(provide field-name)

(define (field-name x) (~a "\"" x "\""))

@; -----------------------------------------------------------------------------

(provide include-it)

(define (include-it file-name)
  (apply verbatim #:indent 4 (element-join (file->lines file-name) "\n")))
